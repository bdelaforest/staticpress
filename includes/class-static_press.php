...
CREATE TABLE `{$this->url_table}` (
 ...
 KEY `last_upload` (`last_upload`)
      ...
      if (preg_match('#\.php$#i', $url['url'])) {
        ...
        if ($url['enable'] == 1) {
          if (preg_match('#^'.preg_quote($plugin_dir).'#i', $url['url'])){
            ...
            // Or parent theme
            $parent_tpl = wp_get_theme();
            $parent_theme = trailingslashit($theme_dir . $parent_tpl->Template);

            if (preg_match('#^('.preg_quote($current_theme).'|'.preg_quote($parent_theme).')#i', $url['url'])) {
              $url['enable'] = 1;
            }
          }
        }
  ...
  private function get_urls(){
    ...
    // Remove 'attachment' post_type
    $pts = get_post_types(array('public' => true));
    if (isset($pts['attachment'])) { unset($pts['attachment']); }
